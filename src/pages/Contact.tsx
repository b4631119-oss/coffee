...
export default function Contact() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { from: 'bot', text: t(language, 'chatGreeting') }
  ]);
  ...
  const handleSendChat = () => {
    ...
    setTimeout(() => {
      setChatMessages(prev => [...prev, { from: 'bot', text: t(language, 'chatReply') }]);
    }, 1000);
    setChatInput('');
  };
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-dark-bg' : 'bg-cream'}`}>
      ...
      <section className="relative py-16 lg:py-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-serif text-4xl lg:text-6xl font-bold text-cream mb-4">{t(language, 'contactTitle')}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-2">
              {submitted ? (
                <div className={`p-8 rounded-2xl text-center ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-lg`}>
                  ...
                  <h2 className={`font-serif text-2xl font-bold mb-2 ${darkMode ? 'text-cream' : 'text-espresso'}`}>{t(language, 'contactSentTitle')}</h2>
                  <p className={darkMode ? 'text-cream/60' : 'text-espresso/60'}>{t(language, 'contactSentDesc')}</p>
                  <button onClick={() => setSubmitted(false)} className="btn-primary mt-6">{t(language, 'contactSendAnother')}</button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className={`p-6 lg:p-8 rounded-2xl ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-lg`}>
                  <h2 className={`font-serif text-2xl font-bold mb-6 ${darkMode ? 'text-cream' : 'text-espresso'}`}>{t(language, 'contactSendMsg')}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                      <label className={`block text-sm font-medium mb-1.5 ${darkMode ? 'text-cream/70' : 'text-espresso/70'}`}>{t(language, 'contactName')} *</label>
                    ...
                    <div>
                      <label className={`block text-sm font-medium mb-1.5 ${darkMode ? 'text-cream/70' : 'text-espresso/70'}`}>{t(language, 'contactEmail')} *</label>
                  ...
                  <div className="mb-4">
                    <label className={`block text-sm font-medium mb-1.5 ${darkMode ? 'text-cream/70' : 'text-espresso/70'}`}>{t(language, 'contactSubject')}</label>
                    <select className={`w-full px-4 py-3 rounded-lg border ${darkMode ? 'bg-dark-surface border-dark-surface text-cream' : 'bg-cream border-beige text-espresso'} focus:outline-none focus:border-caramel`}>
                      {t(language, 'contactSubjects').map((subject: string, i: number) => <option key={i}>{subject}</option>)}
                    </select>
                  </div>
                  <div className="mb-6">
                    <label className={`block text-sm font-medium mb-1.5 ${darkMode ? 'text-cream/70' : 'text-espresso/70'}`}>{t(language, 'contactMessage')} *</label>
                  ...
                  <button type="submit" className="btn-primary flex items-center gap-2">
                    <Send className="w-4 h-4" /> {t(language, 'contactSendBtn')}
                  </button>
                </form>
              )}
            </div>
            ...
            <div className="space-y-6">
              <div className={`p-6 rounded-2xl ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-md`}>
                <h3 className={`font-serif text-lg font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>{t(language, 'contactInfoTitle')}</h3>
              ...
              <div className={`p-6 rounded-2xl ${darkMode ? 'bg-dark-card' : 'bg-beige'} shadow-md`}>
                <h3 className={`font-serif text-lg font-semibold mb-3 ${darkMode ? 'text-cream' : 'text-espresso'}`}>{t(language, 'contactFAQTitle')}</h3>
                <div className="space-y-3">
                  <div>
                    <p className={`text-sm font-medium ${darkMode ? 'text-cream/80' : 'text-espresso/80'}`}>{t(language, 'contactCateringQ')}</p>
                    <p className={`text-xs mt-1 ${darkMode ? 'text-cream/50' : 'text-espresso/50'}`}>{t(language, 'contactCateringA')}</p>
                  </div>
                  <div>
                    <p className={`text-sm font-medium ${darkMode ? 'text-cream/80' : 'text-espresso/80'}`}>{t(language, 'contactParkingQ')}</p>
                    <p className={`text-xs mt-1 ${darkMode ? 'text-cream/50' : 'text-espresso/50'}`}>{t(language, 'contactParkingA')}</p>
                  </div>
                  <div>
                    <p className={`text-sm font-medium ${darkMode ? 'text-cream/80' : 'text-espresso/80'}`}>{t(language, 'contactWiFiQ')}</p>
                    <p className={`text-xs mt-1 ${darkMode ? 'text-cream/50' : 'text-espresso/50'}`}>{t(language, 'contactWiFiA')}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={`mb-4 w-80 rounded-2xl overflow-hidden shadow-2xl ${darkMode ? 'bg-dark-card border border-dark-surface' : 'bg-white border border-beige'}`}>
            <div className="bg-caramel p-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-medium text-sm">{t(language, 'chatTitle')}</span>
              </div>
