...
export default function CartPage() {
  ...
  const applyCoupon = () => {
    if (couponCode.toLowerCase() === 'brew10') {
      ...
      alert('Invalid coupon code. Try BREW10 or COFFEE20');
    }
  };
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-dark-bg' : 'bg-cream'}`}>
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h1 className="font-serif text-3xl font-bold mb-4 text-espresso dark:text-cream">Order Confirmed!</h1>
          <p className={`mb-8 ${darkMode ? 'text-cream/60' : 'text-espresso/60'}`}>
            Thank you for your order! We're preparing your items with care. You'll receive a confirmation email shortly.
          </p>
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${darkMode ? 'bg-dark-bg' : 'bg-cream'}`}>
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h1 className="font-serif text-3xl font-bold mb-4 text-espresso dark:text-cream">Your Cart is Empty</h1>
          <p className={`mb-8 ${darkMode ? 'text-cream/60' : 'text-espresso/60'}`}>
            Looks like you haven't added anything yet. Explore our shop and find your perfect brew!
          </p>
        ...
        {!showCheckout ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.map(item => (
                <div key={item.id} className={`flex gap-4 p-4 rounded-xl ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-sm`}>
                  <img src={item.image} alt={item.name} className="w-20 h-20 lg:w-24 lg:h-24 rounded-lg object-cover" />
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className={`font-serif font-semibold ${darkMode ? 'text-cream' : 'text-espresso'}`}>{item.name}</h3>
                        <p className={`text-sm ${darkMode ? 'text-cream/50' : 'text-espresso/50'}`}>{item.category}</p>
                      </div>
            ...
            <div className={`p-6 rounded-xl h-fit sticky top-24 ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-lg`}>
              <h3 className={`font-serif text-xl font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>Order Summary</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <form onSubmit={handleCheckout} className="lg:col-span-2 space-y-6">
              <div>
                <h3 className={`font-serif text-xl font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>Contact Information</h3>
                <input
                  type="email"
                  required
              ...
              <div>
                <h3 className={`font-serif text-xl font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>Shipping Address</h3>
              ...
              <div>
                <h3 className={`font-serif text-xl font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>Payment</h3>
                <div className={`p-6 rounded-xl border-2 border-dashed ${darkMode ? 'border-dark-surface' : 'border-beige'} text-center`}>
                  ...
                  <p className={`text-sm ${darkMode ? 'text-cream/60' : 'text-espresso/60'}`}>
                    Stripe / PayPal integration would go here.<br/>
                    <span className="text-xs">This is a demo — no real payment is processed.</span>
                  </p>
                </div>
              </div>
            ...
            {/* Mini Summary */}
            <div className={`p-6 rounded-xl h-fit sticky top-24 ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-lg`}>
              <h3 className={`font-serif text-lg font-semibold mb-4 ${darkMode ? 'text-cream' : 'text-espresso'}`}>Your Order</h3>
