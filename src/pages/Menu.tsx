    ...
    <div className={`min-h-screen ${darkMode ? 'bg-dark-bg' : 'bg-cream'}`}>
      ...
      <section className="relative py-16 lg:py-24 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-serif text-4xl lg:text-6xl font-bold text-cream mb-4">{t(language, 'menuTitle')}</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                placeholder={t(language, 'menuSearch')}
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
              <button onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex items-center gap-1 px-3 py-2 rounded-full border border-caramel/30 text-caramel text-sm">
                <Filter className="w-4 h-4" /> {t(language, 'menuFilter')}
              </button>
              <div className={`${showFilters ? 'flex' : 'hidden'} sm:flex gap-2 flex-wrap`}>
                {categories.map(cat => (
                  ...
                  >
                    {cat === 'All' ? t(language, 'menuAll') : cat}
                  </button>
                ))}
              </div>
            </div>
      ...
      <section className="py-12 lg:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className={`mb-6 text-sm ${darkMode ? 'text-cream/50' : 'text-espresso/50'}`}>
            {t(language, 'menuShowing')} {filteredProducts.length} {filteredProducts.length !== 1 ? t(language, 'menuItems') : ''}
          </p>

          {filteredProducts.length === 0 ? (
            <div className="text-center py-16">
              <p className={`text-lg ${darkMode ? 'text-cream/60' : 'text-espresso/60'}`}>{t(language, 'menuNoItems')}</p>
            </div>
          ) : (
              ...
              {filteredProducts.map((product, index) => (
                <div key={product.id} className={`card-hover rounded-2xl overflow-hidden ${darkMode ? 'bg-dark-card' : 'bg-white'} shadow-md animate-fade-in-up`} style={{ animationDelay: `${index * 0.05}s` }}>
                  ...
                  <div className="p-4">
                    ...
                    <Link to={`/product/${product.id}`}>
                      <h3 className={`font-serif text-lg font-semibold mb-1 hover:text-caramel transition-colors ${darkMode ? 'text-cream' : 'text-espresso'}`}>{getProductName(product.id, language, product.name)}</h3>
                    </Link>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <ShoppingBag className="w-3.5 h-3.5" /> {t(language, 'menuAddToCart')}
                      </button>
                    </div>
                  </div>
                </div>
